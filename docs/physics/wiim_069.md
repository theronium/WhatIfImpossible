...
跳躍先制御の部分的な解として、アンキロン（wiim_022）との組み合わせが考えられる。アンキロンが核反応点の計量テンソルを固定し、コーラ粒子がその計量座標を参照して跳躍先を決定するという間接的な構成だ。制御精度の問題が計量レベルで抑えられる可能性はあるが、サブオングストローム精度の座標指定を計量操作で実現できるかは未解決のままだ（→ wiim_070）。
...
- [wiim_070](wiim_070.md) — 核融合生成物の即時中性化——アルファ固着をゼロにすればミュオン触媒核融合は実用化するか（本記事のアプローチを核融合に特化して深掘り）
- [wiim_071](wiim_071.md) — 中性核融合生成物の世界——電荷ゼロで生まれたヘリウムが変える物理・工学・文明（帰結の考察）
