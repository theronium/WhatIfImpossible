...
ただし「切れない」という表現は正確ではない。フラックスチューブに十分なエネルギーを注ぐと紐は**実際に切れる**——しかし切れた両端に直ちに新しいクォーク・反クォーク対が生成されて短い紐が二本に増える（ハドロン化・弦断片化）。「切れない」のではなく「**切ると複製される**」が正確な挙動だ。この区別は本記事の思考実験において重要な含意を持つ。切断抵抗だけでなく「切断によって自己複製する」性質を「切れないエネルギー紐」の実装条件に組み込むと、単なる強固な構造ではなく質的に異なる概念——切断コストが指数的に膨らむ構造——として再定義できる。
