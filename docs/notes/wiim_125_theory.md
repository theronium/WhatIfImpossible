...
> **余談**：2番目の「くりこみ群の走り」で登場する有効ポテンシャルは、微視的な量子ゆらぎをスケールごとに積み上げた（くりこんだ）先に得られる量である。くりこみ群は素粒子の結合定数の走りだけでなく、相転移の臨界現象やカオスへの周期倍分岐が示す自己相似構造も同じ数式で説明する、物理学の中でも特に応用範囲の広い枠組みである。詳しくは [feynman_path_integral_unification.md](feynman_path_integral_unification.md) を参照。
