...
id: wiim_039
category: quantum
tags: [カシミール効果, 量子永久機関, ゼロ点エネルギー, 非対称カシミール板, コーラ粒子, パランティ粒子, 余剰次元, デコヒーレンス, 真空エネルギー, エントロピー輸送]
date: 2026-03-25
---
...
コーラ粒子板が仮想光子を余剰次元に排出し続けるためには、その余剰次元のエントロピーが増大し続けなければならない。局所的にエネルギーを取り出せても、宇宙全体の帳簿は必ず赤字になる。

ここで「エネルギーを取り出す」という表現を補足する。通常のカシミール効果では、板を近づける際の機械的仕事がカシミール力によって別の形に**変換**されるのであり、真空から正味のエネルギーが「取り出される」わけではない——板を離す時には外部からエネルギーを投入しなければならない。非対称カシミール板のサイクルも同様で、「取り出し」ではなく**変換効率の非対称性をサイクルとして回せるか**が問われている。この区別は永久機関の成否を判断する上で本質的であり、変換サイクルが外部仕事なしに持続するかどうかが技術的焦点だ。パランティ粒子（g161）板を使う場合は、さらに根本的な障壁がある——安定した負エネルギー状態は現行の量子場理論に存在せず、パランティ粒子板を生成する時点で真空の安定性を脅かす（wiim_038と同じ問題が再現される）。

### 技術的限界

コーラ粒子は波動関数（g164）に従わない粒子として定義されるため、板そのものはデコヒーレンス（g173）の影響を受けない——デコヒーレンスは波動関数の重ね合わせが環境との相互作用で失われる現象であり、波動関数に従わないコーラ粒子には原理的に適用されないからだ。

しかし問題は**板と仮想光子の接続部**に移る。余剰次元に排出されるのはあくまで通常の仮想光子であり、仮想光子は波動関数に従う。コーラ粒子板が仮想光子を余剰次元へ「引き渡す」際の結合機構——通常の量子場とコーラ粒子場の境界——は未知であり、この界面での効率損失や安定性が技術的障壁として残る。
...
### パランティ粒子板による引力・斥力非対称サイクル

パランティ粒子（g161）板を使うと、さらに能動的なサイクルが構成できる可能性がある。

| フェーズ | 通常板 | パランティ粒子板 | 力の方向 |
|---------|--------|-----------|---------|
| 接近相 | 仮想光子を反射 | 仮想光子を反転吸収（負エネルギー） | 引力（通常カシミール） |
| 離隔相 | 仮想光子を反射 | 反転放出で逆カシミール力 | 斥力（自然に押し離される） |

接近時にエネルギーを収穫し、離隔時にパランティ粒子板が自然に板を押し返す——外部からリセットエネルギーを投入しなくても板が自動で元の位置に戻るサイクルだ。ただしこれはパランティ粒子の安定生成という、現行物理では不可能な前提を要する。

### カシミールフォージとの接続

カシミールフォージ（wiim_023）はカシミール効果を利用してエキゾチック物質を生成する設備として設計されている。非対称カシミール板の製造にも応用できると考えられる——フォージの出力物として「コーラ粒子含浸板」や「パランティ粒子コーティング板」を生産し、エンジンの消耗部品として供給するサプライチェーンが成立しうる。
...
- [wiim_038](../physics/wiim_038.md) — 静かな対消滅（パランティ粒子板が直面する同一の障壁）
