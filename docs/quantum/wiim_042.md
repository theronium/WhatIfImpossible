...
この問題は「測定順序を変える」「弱測定を使う」といった手法で部分的に緩和できる可能性があるが、根本的には解消されない。弱測定は系を「少ししか乱さない」が情報量も「少ししか得られない」というトレードオフを持ち、Δφ の完全読み出しには原理的に届かない。**本記事はこの「見ずに測る矛盾」を未解決の前提条件として留保した上で、Δφ が読み出せると仮定した場合の帰結を論じている。**
...
### 定義できるクオリアと、真のクオリアの断絶

この検知機が最終的に示すのは、次の非対称性だ。

我々が「クオリア = 創発の主観的側面」と定義した以上、Δφ ≠ 0 によって**我々の定義したクオリア**は検知できる。しかしその定義が哲学的に真のクオリア——チャーマーズやネーゲルが問う「体験の不可還元性」——と同じものかどうかは、その定義からは証明できない。

定義の正しさを、定義から導かれる方法で検証することはできないからだ。

あらゆる操作的定義は機能的記述に落ちてしまい、「なぜそこに体験があるのか」という問いには届かない。これはハードプロブレムの言い換えではなく、ハードプロブレムが**どこに残り続けるか**の正確な所在だ。

> 測定できないものを定義し、測定できるものだけを捉えた——それが科学とクオリアの関係の全体かもしれない。
