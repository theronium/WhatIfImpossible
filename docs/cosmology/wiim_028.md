...
重力子も光速で伝播する——光子に対して速度優位はない。本設計における重力子の優位性は**透過性**にある。光子はチャネル外の通常空間では赤方偏移・吸収・散乱によって減衰するが、重力子はいかなる物質にも吸収されず素通りする。「先触れ信号」として重力子を使う理由は速度ではなく、チャネルなしで通常空間を届きうる唯一の搬送波だからだ。

FTL はあくまでエキゾチック物質チャネルの時空歪曲に由来する。重力子単体を通常空間で使う限りは光速の制約を受け、FTL通信にはならない。重力子がチャネル内でも使われる場合は光子と同様にFTLの恩恵を受けるが、その場合は「重力子だから先着する」のではなく「チャネル内の近道を通るから先着する」という整理になる。
...
組み合わせると自然な設計が浮かぶ：重力子で「チャネルを見つける」先触れ信号を送り、受信局が位置を確認してからチャネルを開通し、光子で本文を送る。ただしこの先触れ信号は「速いから先着する」のではなく「通常空間を透過できるから別経路で届く」という意味であり、到着時刻は通常の光速伝播と同じだ。重力子がチャネル内を通れば光子と同速でFTLの恩恵を受けるが、その場合は透過性優位が失われる——重力子をチャネル外で使う設計でのみ、この役割分担は意味を持つ。
