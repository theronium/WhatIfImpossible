...
- **Mermaidのノードラベル**では丸括弧`()`・コロン`:`・半角スラッシュ`/`を避ける。ノード形状の指定と誤認されてパースエラーになることがある（例：`V(φ)`のような数式的表記、`wiim_125:`のような接頭コロン、`1/2`のような分数表記）。数式風の内容は「ポテンシャルの形」のような言葉に置き換えるか、どうしても必要な場合はラベル全体をダブルクォートで囲む
- ラベル全体をダブルクォートで囲んだ場合、**そのラベルの中に半角の`"`を再度使わない**（囲みが途中で終わったとパースされ、続きが構文エラーになる）。また**ダブルクォートで囲んでいない角括弧ラベルの中にも半角`"`を書かない**——`B4[複数世代の"ゲノム"比較ができず]`のような例は同じ理由でエラーになる。強調したい語は全角の鉤括弧「」で囲む
